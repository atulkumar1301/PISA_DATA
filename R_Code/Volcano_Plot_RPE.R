...
df <- fread ("~/OneDrive - University of Eastern Finland/Projects/Astrid_Subrizi_PISA_Data/Plots/Full_Result_RPE_4.txt", sep = "\t", fill = TRUE)
...
p <- p + geom_hline (aes(yintercept=-log10(0.05), linetype = "p-value 0.05", col="black")) +
  geom_hline (aes (yintercept=-log10(0.00000710328), linetype = "FDR p-value 0.05", col="#D55E00")) +
...
p <- p +
  theme(legend.position="bottom",
        ...
        panel.background = element_blank()) + labs(title=expression("RPE-4 vs Control"))
p
